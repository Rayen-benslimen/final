...
export class FicheComponent {
      ...
      ngOnInit(): void {
        ...
        this.getFiches(this.authenticationService.currentUserValue['userid']);
        this.getCorps();
        this.getBanks();
  ...
  get rib(): AbstractControl {
      ...
      if (this.formulaire.valid) {
        ...
        this.ficheService.createFiche(fiche).subscribe(
          data => {
            ...
            this.router.navigateByUrl('fiche/', {skipLocationChange: true}).then(() => this.router.navigate(['client/fiche/']));
    ...
    getFiches(userid : number) {
      this.ficheService.getFiches(userid).subscribe(
        fiches => {
